...
public class ClassRegistrationUtils {
    ...
    public static void loadListeners(String packageName) {
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (isListener(clazz)) {
                try {
                    MineraveLaserTag.getMlt().getServer().getPluginManager().registerEvents((Listener) clazz.newInstance(), MineraveLaserTag.getMlt());
    ...
    public static Collection<Class<?>> getClassesInPackage(String packageName) {
        ...
        CodeSource codeSource = MineraveLaserTag.getMlt().getClass().getProtectionDomain().getCodeSource();
