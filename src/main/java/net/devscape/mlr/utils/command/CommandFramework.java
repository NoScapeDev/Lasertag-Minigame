...
public class CommandFramework implements CommandExecutor {
    ...
    private MineraveLaserTag plugin;
    private Map<String, Entry<Method, Object>> commandMap = new HashMap<>();
    private CommandMap map;

    public CommandFramework(MineraveLaserTag plugin) {
        this.plugin = plugin;
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage(format("&cYou don't have permissions to perform this."));
                    return true;
                }
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(format("&cThis command can only be executed in game."));
                    return true;
                }
